...
public class PortfolioService {
    ...
    public PortfolioDTO create(PortfolioDTO dto) {
        ...
        if (dto.getSymbolId() == null || dto.getSymbolId().isBlank()) throw new IllegalArgumentException("symbolId is required");

        User user = userRepo.findById(dto.getUserId())
                .orElseThrow(() -> new ResourceNotFoundException("User not found: " + dto.getUserId()));

        Symbol symbol = symbolRepo.findById(dto.getSymbolId())
                .orElseThrow(() -> new ResourceNotFoundException("Symbol not found: " + dto.getSymbolId()));
    ...
    public PortfolioDTO update(Integer id, PortfolioDTO dto) {
        Portfolio existing = portfolioRepo.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Portfolio not found: " + id));
        ...
        if (dto.getSymbolId() != null && !dto.getSymbolId().equals(existing.getSymbol().getSymbolId())) {
            Symbol symbol = symbolRepo.findById(dto.getSymbolId())
                    .orElseThrow(() -> new ResourceNotFoundException("Symbol not found: " + dto.getSymbolId()));
